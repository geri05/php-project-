<?php
/*
 * reserve.php — Creates a timed parking reservation for the authenticated user.
 * Handles auto-cleanup of expired reservations, concurrency locking, and
 * automatic vehicle assignment before inserting the reservation.
 */
...
if ($duration_hours > 6) $duration_hours = 6;
...
try {
    ...
    if ($check->fetch()) {
        echo json_encode([
            'success' => false,
            'error'   => 'You already have an active reservation or session. Please end it before creating a new one.'
        ]);
        exit;
    }
    ...
    $reservation_id = (int)$stmt->fetchColumn();
    ...
    hardResetConnection($pdo);
    error_log('[reserve.php] ' . $e->getMessage() . ' @ ' . $e->getFile() . ':' . $e->getLine());
    echo json_encode([
        'success' => false,
